/**
 * File:        apps/backend/src/core/application/use-cases/contacts/add-tag/add-tag.handler.ts
 * Module:      Contacts Use Cases
 * Purpose:     Add a tag to a contact
...
export class AddTagHandler {
  ...
  async execute(command: AddTagCommand): Promise<Result<string, ApplicationError>> {
    ...
    const result = contact.addTag(command.input.tag);
    if (result.isErr()) {
